...
public:
    ...
    bool updateFromPlainBle(int leftLevel, bool isLeftCharging, int rightLevel, bool isRightCharging, int caseLevel, bool isCaseCharging, bool isLeftPrimary, bool podInCase, bool isHeadset)
    {
        if (isHeadset) {
            int lvl = (leftLevel >= 0) ? leftLevel : ((rightLevel >= 0) ? rightLevel : caseLevel);
            if (lvl >= 0) {
                primaryPod = Component::Headset;
                states[Component::Headset] = {static_cast<quint8>(lvl), isLeftCharging ? BatteryStatus::Charging : BatteryStatus::Discharging};
            }
        } else {
            if (leftLevel >= 0) {
            ...
            secondaryPod = isLeftPrimary ? Component::Right : Component::Left;
        }
        emit batteryStatusChanged();
        emit primaryChanged();
        return true;

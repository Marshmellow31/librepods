...
{
    LOG_DEBUG("BLE scan finished, restarting for continuous scanning...");
    discoveryAgent->start(QBluetoothDeviceDiscoveryAgent::LowEnergyMethod);
}

void BleManager::onErrorOccurred(QBluetoothDeviceDiscoveryAgent::Error error)
{
    LOG_ERROR("BLE scan error occurred: " << error << " (" << discoveryAgent->errorString() << ")");
    QTimer::singleShot(5000, this, [this]() {
        LOG_INFO("Retrying BLE scan...");
        startScan();
    });
}

...
public:
        ...
        , m_systemSleepMonitor(new WindowsSleepMonitor(this))
    {
        ...
        if (m_systemSleepMonitor->initialize())
        {
            ...
            connect(m_systemSleepMonitor, &WindowsSleepMonitor::systemWakingUp, this, &AirPodsTrayApp::onSystemWakingUp);
        }
        else
        {
...
private slots:
    ...
    {
        LOG_INFO("BLE device detected: Address=" << device.address
                 << ", Model=" << static_cast<int>(device.modelName)
                 << ", L=" << device.leftPodBattery << "%" << (device.leftCharging ? "(charging)" : "")
                 << ", R=" << device.rightPodBattery << "%" << (device.rightCharging ? "(charging)" : "")
                 << ", Case=" << device.caseBattery << "%" << (device.caseCharging ? "(charging)" : "")
                 << ", InEar(L/R)=(" << device.isLeftPodInEar << "/" << device.isRightPodInEar << ")"
                 << ", Primary=" << (device.primaryLeft ? "Left" : "Right")
                 << ", InCase=" << device.isThisPodInTheCase);

        bool hasIrk = !m_deviceInfo->magicAccIRK().isEmpty();
        bool irkValid = hasIrk && BLEUtils::isValidIrkRpa(m_deviceInfo->magicAccIRK(), device.address);
        ...
        if (irkValid) {
            ...
            if (m_deviceInfo->deviceName().isEmpty()) {
                m_deviceInfo->setDeviceName(device.name);
            }
            m_deviceInfo->getBattery()->updateFromPlainBle(
                device.leftPodBattery, device.leftCharging,
                device.rightPodBattery, device.rightCharging,
                device.caseBattery, device.caseCharging,
                device.primaryLeft, device.isThisPodInTheCase,
...
public:
    ...
    void loadMainModule() {
        parent->load(QUrl(QStringLiteral("qrc:/windows/Main.qml")));
